...
def generate_flit_trace(tensor_trace_path, flit_trace_path, flit_size, injection_rate, radix):
    """
    Generates a flit trace file from a tensor trace file.
    ...
    for _, row in df.iterrows():
        ...
        for i in range(num_flits):
            is_head = 1 if i == 0 else 0
            is_tail = 1 if i == (num_flits - 1) else 0
    ...
    print(f"Successfully generated {flit_trace_path} with {len(flits)} flits.")
...
def main():
    ...
    tensor_trace_file = '/pool0/suhyeong/06_3D_interconnection/stream/exp2/tensor_trace.txt'
    flit_trace_file = '/pool0/suhyeong/06_3D_interconnection/stream/exp2/flit_trace.txt'

    generate_flit_trace(tensor_trace_file, flit_trace_file, flit_size, injection_rate, radix)
